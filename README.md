...
Once the tests compiled, I was able to write a test for the ARPService to test that it was returning the MAC address in the correct format. Turns out, it wasn't! It was providing dashes instead of colons. So, writing the tests was actually useful to catch this error.
...
I found out that MQTTNet supports attaching an interceptor callback to the connect and subscribe events, so a little bit of debugging time here paid dividends since I now had a mechanism to (more or less) ensure that the device requesting a connection was actually the one I was expecting. I probably could have gotten the token piece working but realized it didn't really make sense since whoever got access to the topic in the first place would be able to just grab the key. I don't know enough about cryptography to really properly do this and was advised against going the full nine yards. Hopefully showing how it _would_ have worked is sufficient.
